...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    return json({ ok: true, products: results.map(p => ({ ...p, low: p.low===1, active: p.active===1 })) });
  ...
  try {
    ...
    const result = await env.DB.prepare(`
      INSERT INTO products
        (name, stor, ars, ars_old, usd, badge, btxt, stock_qty, low, stock_label, color, cat, url, img, active, offer_end)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
  ...
  try {
    ...
    await env.DB.prepare(`
      UPDATE products SET
        ...
        active=?, offer_end=?, updated_at=datetime('now')
      WHERE id=?
    `).bind(
      b.name.trim(), (b.stor||'').trim(),
      b.ars     ? parseInt(b.ars)     : null,
